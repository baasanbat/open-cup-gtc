...
int n, m;
char a[300][300+1];
llint mask[302][302][4]; // (r, s)-ees 64 bit dotorh utgaar d chiglel

const int dr[4] = { 1, 0, -1, 0 }; //v, >, ^, <
...
int main( void ) {
   ...
   for( int r = 0; r < n; ++r ) {
      for( int c = 0; c < m; ++c )
         for( int x = 0; x < 2; ++x ) {
            int tmp_max = min( min( r+1, c+1 ), min( n-r, m-c ) );
            if ( 2*tmp_max+x <= ret ) {
               // zuv bish square tul endees tsaash haigaad hereggui
               continue;
            }
